...
object RuntimeStub {
    ...
    fun exec(runtime: Runtime, command: String): Process? {
        AppLogger.i("SecuritySandbox", "Blocked Runtime.exec($command)")
        return null
    }

    @JvmStatic
    fun exec(runtime: Runtime, cmdarray: Array<String>): Process? {
        AppLogger.i("SecuritySandbox", "Blocked Runtime.exec(${cmdarray.joinToString()})")
        return null
    }
    ...
    fun loadLibrary(runtime: Runtime, libname: String) {
        AppLogger.i("SecuritySandbox", "Blocked Runtime.loadLibrary($libname)")
    }

    @JvmStatic
    fun load(runtime: Runtime, filename: String) {
        AppLogger.i("SecuritySandbox", "Blocked Runtime.load($filename)")
    }

    @JvmStatic
    fun exit(runtime: Runtime, status: Int) {
        AppLogger.i("SecuritySandbox", "Blocked Runtime.exit($status)")
    }

    @JvmStatic
    fun halt(runtime: Runtime, status: Int) {
        AppLogger.i("SecuritySandbox", "Blocked Runtime.halt($status)")
    }
