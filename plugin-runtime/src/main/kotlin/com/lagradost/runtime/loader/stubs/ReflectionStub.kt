...
object ReflectionStub {
    ...
    private fun isDangerous(className: String, methodName: String? = null): Boolean {
        ...
        if (className == "com.lagradost.runtime.loader.SafePluginClassLoader" ||
            className == "com.lagradost.cloudstream3.loader.SafePluginClassLoader") return true
    ...
    fun invoke(method: Method, obj: Any?, args: Array<Any?>?): Any? {
        if (isDangerous(method.declaringClass.name, method.name)) {
            AppLogger.i("SecuritySandbox", "Blocked reflection invoke on ${method.declaringClass.name}.${method.name}")
    ...
    fun get(field: Field, obj: Any?): Any? {
        if (isDangerous(field.declaringClass.name)) {
            AppLogger.i("SecuritySandbox", "Blocked reflection get on ${field.declaringClass.name}.${field.name}")
    ...
    fun set(field: Field, obj: Any?, value: Any?) {
        if (isDangerous(field.declaringClass.name)) {
            AppLogger.i("SecuritySandbox", "Blocked reflection set on ${field.declaringClass.name}.${field.name}")
    ...
    fun newInstance(constructor: Constructor<*>, args: Array<Any?>?): Any {
        if (isDangerous(constructor.declaringClass.name)) {
            AppLogger.i("SecuritySandbox", "Blocked reflection newInstance on ${constructor.declaringClass.name}")
    ...
    fun setAccessible(accessibleObject: java.lang.reflect.AccessibleObject, flag: Boolean) {
        ...
        if (declaringClass != null && isDangerous(declaringClass.name)) {
            AppLogger.i("SecuritySandbox", "Blocked setAccessible on ${declaringClass.name}")
