// @PortSource(file = "upstream/app/src/main/java/com/lagradost/cloudstream3/utils/DataStoreHelper.kt", upstreamCommit = "caeec18")
...
object DataStoreHelper {
    ...
    fun setAccount(account: Account) {
        ...
        DesktopDataStore.currentAccount = account.keyIndex.toString()
    ...
    fun setViewPos(id: Int?, pos: Long, dur: Long) {
        if (id == null) return
        val (normalizedPos, normalizedDur) = if (dur in 1 until 30_000L) {
            (pos * 1000L) to (dur * 1000L)
        } else {
            pos to dur
        }
        if (normalizedDur < 30_000) return // too short
        setKey("$currentAccount/$VIDEO_POS_DUR", id.toString(), PosDur(normalizedPos, normalizedDur))
    }
    ...
    fun setViewPosAndResume(id: Int?, position: Long, duration: Long, currentEpisode: Any?, nextEpisode: Any?) {
        ...
        val percentage = if (normalizedDur > 0) normalizedPos * 100L / normalizedDur else 0L
        ...
        if (resumeMeta == null && nextEp) {
            ...
            when (val newMeta = currentEpisode) {
                ...
                is WatchHistory -> {
                    val pId = newMeta.parentId?.toIntOrNull() ?: newMeta.parentId?.hashCode()
                    removeLastWatched(pId)
                }
            }
        } else {
            // save resume
            when (resumeMeta) {
                ...
                is WatchHistory -> {
                    val pId = resumeMeta.parentId?.toIntOrNull() ?: resumeMeta.parentId?.hashCode()
                    val epId = resumeMeta.episodeId?.toIntOrNull() ?: resumeMeta.episodeId?.hashCode() ?: resumeMeta.url.hashCode()
                    setLastWatched(
                        pId,
                        epId,
                        resumeMeta.episode,
    ...
    fun getViewPos(id: Int?): PosDur? {
        if (id == null) return null
        val posDur = getKey<PosDur>("$currentAccount/$VIDEO_POS_DUR", id.toString(), null) ?: return null
        return if (posDur.duration in 1 until 30_000L) {
            PosDur(posDur.position * 1000L, posDur.duration * 1000L)
        } else {
            posDur
        }
    }
