// @PortSource(file = "upstream/app/src/main/java/com/lagradost/cloudstream3/syncproviders/AuthRepo.kt", upstreamCommit = "9feeaedee64b3efc150a3f05df18f955abe170d0")
...
abstract class AuthRepo(open val api: AuthAPI) {
    ...
    protected suspend fun freshAuth(): AuthData? {
        ...
        val mutex = refreshMutexes.computeIfAbsent(idPrefix) { Mutex() }
        return mutex.withLock {
            // Re-check after acquiring lock (double-checked locking)
            val currentData = authData() ?: return@withLock null
            if (!currentData.token.isAccessTokenExpired()) {
                return@withLock currentData
            }
            val newToken = api.refreshToken(currentData.token) ?: return@withLock null
            val newAuth = AuthData(user = currentData.user, token = newToken)
            refreshUser(newAuth)
            newAuth
        }
    }
