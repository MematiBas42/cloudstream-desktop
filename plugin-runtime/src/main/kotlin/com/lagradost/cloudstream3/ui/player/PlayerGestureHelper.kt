// @PortSource(file = "upstream/app/src/main/java/com/lagradost/cloudstream3/ui/player/PlayerGestureHelper.kt", upstreamCommit = "caeec18")
 ...
 *
 * Instantiated and owned by [PlayerView]; accessed from host fragments/views via the delegate
 * properties [PlayerView] exposes.
 */
@OptIn(UnstableApi::class)
class PlayerGestureHelper(private val playerView: PlayerView = PlayerView()) {
    ...
    private val context: Context get() = playerView.context ?: Context()
     ...
     * the app. Note that this will make it a bit weird when using loudness in PiP, then returning
    ...
    fun updateBrightnessOverlayBounds() {
        ...
        val lp = overlay.layoutParams as? ViewGroup.LayoutParams
        if (lp == null) {
    ...
    fun calculateNewTime(startTime: Long?, touchStart: Vector2?, touchEnd: Vector2?): Long? {
...
/**
 * Desktop Player Gesture Adapter
 *
 * Implements mouse-first interaction model as defined in CLAUDE.md Section 1 and Section 5.6:
 * - Mouse scroll-wheel volume adjustment (smooth +/- 0.05f steps)
 * - Shift + Mouse scroll-wheel scrubbing (+/- seekStepMs)
 * - Volume slider linear & boosted dragging (0% to 200%)
 * - Timeline slider drag scrubbing with duration mapping
 * - 3-zone double-tap handling (left 25% rewind, right 25% forward, center 50% toggle)
 * - Hold-to-speedup (500ms long press -> 2.0x temporary speedup)
 */
data class DesktopPlayerGestureAdapter(
    var isMuted: Boolean = false,
    var previousVolume: Float = 1.0f,
    var currentVolume: Float = 1.0f,
    var currentPlaybackSpeed: Float = 1.0f,
    var originalPlaybackSpeed: Float = 1.0f,
    var isHoldingSpeedUp: Boolean = false,
    var isSeeking: Boolean = false,
    var seekPositionMs: Long = 0L,
    var fastForwardTimeMs: Long = 10_000L
) {
    sealed class DesktopScrollResult {
        data class VolumeChange(val newVolume: Float, val delta: Float) : DesktopScrollResult()
        data class TimeSeek(val targetPositionMs: Long, val deltaMs: Long) : DesktopScrollResult()
        object None : DesktopScrollResult()
    }

    sealed class DesktopTapAction {
        data class Rewind(val targetPositionMs: Long) : DesktopTapAction()
        data class FastForward(val targetPositionMs: Long) : DesktopTapAction()
        object ToggleFullscreen : DesktopTapAction()
        object TogglePlayPause : DesktopTapAction()
    }
    ...
    fun handleMouseScroll(
        ...
        if (scrollDeltaY == 0f) return DesktopScrollResult.None

        if (isShiftPressed) {
            val deltaMs = if (scrollDeltaY < 0f) fastForwardTimeMs else -fastForwardTimeMs
            val targetMs = (currentPositionMs + deltaMs).coerceIn(0L, durationMs.coerceAtLeast(0L))
            seekPositionMs = targetMs
            return DesktopScrollResult.TimeSeek(targetMs, deltaMs)
        } else {
            val delta = if (scrollDeltaY < 0f) 0.05f else -0.05f
            val newVol = (currentVolume + delta).coerceIn(0f, 1f)
            currentVolume = newVol
            if (newVol > 0f) isMuted = false
            return DesktopScrollResult.VolumeChange(newVol, delta)
        }
    }

    fun handleVolumeDrag(fraction: Float, allowBoost: Boolean = false): Float {
        val maxVol = if (allowBoost) 2.0f else 1.0f
        val clamped = fraction.coerceIn(0f, maxVol)
        currentVolume = clamped
        if (clamped > 0f) isMuted = false
        return clamped
    }

    fun handleTimelineScrub(fraction: Float, durationMs: Long): Long {
        val clampedFraction = fraction.coerceIn(0f, 1f)
        val targetMs = (clampedFraction * durationMs.coerceAtLeast(0L)).toLong()
        seekPositionMs = targetMs
        return targetMs
    }

    fun handleDoubleTapZones(
        clickX: Float,
        containerWidth: Float,
        currentPositionMs: Long,
        durationMs: Long
    ): DesktopTapAction {
        if (containerWidth <= 0f) return DesktopTapAction.ToggleFullscreen
        val leftZoneEnd = containerWidth * 0.25f
        val rightZoneStart = containerWidth * 0.75f

        return when {
            clickX < leftZoneEnd -> {
                val targetMs = (currentPositionMs - fastForwardTimeMs).coerceAtLeast(0L)
                DesktopTapAction.Rewind(targetMs)
            }
            clickX > rightZoneStart -> {
                val targetMs = (currentPositionMs + fastForwardTimeMs).coerceAtMost(durationMs.coerceAtLeast(0L))
                DesktopTapAction.FastForward(targetMs)
            }
            else -> DesktopTapAction.ToggleFullscreen
        }
    }

    fun handleHoldSpeedUp(isHolding: Boolean): Float {
        return if (isHolding) {
            if (!isHoldingSpeedUp) {
                originalPlaybackSpeed = currentPlaybackSpeed
            ...
            if (isHoldingSpeedUp) {
                currentPlaybackSpeed = originalPlaybackSpeed
...
fun PlayerGestureHelper.toDesktopAdapter(): DesktopPlayerGestureAdapter {
    return DesktopPlayerGestureAdapter(
        currentVolume = currentRequestedVolume,
        fastForwardTimeMs = fastForwardTime
    )
}
