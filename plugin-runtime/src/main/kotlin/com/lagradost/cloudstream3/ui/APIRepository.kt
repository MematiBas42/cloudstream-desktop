// @PortSource(file = "upstream/app/src/main/java/com/lagradost/cloudstream3/ui/APIRepository.kt", upstreamCommit = "caeec18")
...
class APIRepository(val api: MainAPI) {
    companion object {
        ...
        /**
         * Synchronizes plugin providers and custom user clone sites into APIHolder.apis.
         * Invoked on application startup and plugin reload lifecycle events.
         */
        fun syncProviders() {
            ioSafe {
                pluginsLock.withLock {
                    APIHolder.allProviders.withLock {
                        try {
                            CloudStreamApp.getKey<Array<CustomSite>>(USER_PROVIDER_API)?.let { list ->
                                list.forEach { custom ->
                                    APIHolder.allProviders.firstOrNull {
                                        it::class.simpleName == custom.parentClassName
                                    }?.let { parent ->
                                        val newInstance = parent::class.createInstance().apply {
                                            name = custom.name
                                            lang = custom.lang
                                            mainUrl = custom.url.trimEnd('/')
                                            canBeOverridden = false
                                        }
                                        APIHolder.allProviders.add(newInstance)
                                    }
                                }
                            }
                            APIHolder.apis = AtomicList(APIHolder.allProviders.distinctBy {
                                it.lang + it.name + it.mainUrl + it::class.qualifiedName
                            })
                            APIHolder.apiMap = null
                            AppLogger.i("APIRepository", "syncProviders: synchronized ${APIHolder.apis.size} providers.")
                        } catch (e: Exception) {
                            logError(e)
                        }
                    }
